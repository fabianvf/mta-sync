...
# This script merges upstream changes into midstream repositories and updates git submodules
...
Sync and merge upstream changes into midstream repositories, updating git submodules as needed.
...
EXAMPLES:
    # Merge and push all repos on default branch
    $0

    # Merge specific repo without committing or pushing
...
reset_to_midstream() {
    ...
    if git show-ref --verify --quiet "refs/remotes/midstream/$branch"; then
        print_status "Switching to and resetting branch to midstream/$branch..."
        # First checkout/create the branch, then reset it
...
for REPO in $REPOS; do
    ...
    # Perform the merge if needed
    if [ "$NEEDS_UPDATE" = true ]; then
        print_status "Merging upstream/origin/$BRANCH..."
        if git merge "origin/$BRANCH" --no-edit; then
            print_status "Merge completed successfully"

            # After merge, update submodules to latest remote if they exist
            if [ -f ".gitmodules" ]; then
                print_status "Updating submodules after merge..."
            ...
            print_status "To abort the merge, run:"
            echo "  cd $REPO && git merge --abort"
            print_status "To continue after resolving conflicts, run:"
            echo "  cd $REPO && git add . && git merge --continue"
            cd ..
            FAILED_REPOS="$FAILED_REPOS $REPO"
            echo ""
            continue
        ...
        # Only update submodules if no merge was needed but submodules are out of sync
        print_status "Updating submodules (no merge needed)..."
        update_submodules "$REPO" "$BRANCH"
    fi
    ...
    if [ "$NEEDS_UPDATE" = true ] || [ "$NEEDS_SUBMODULE_UPDATE" = true ]; then
        if $PUSH_CHANGES; then
            print_status "Pushing to midstream/$BRANCH..."
            if git push midstream "$BRANCH"; then
            ...
            echo "  cd $REPO && git push midstream $BRANCH"
        fi
    fi
...
# Summary
echo "========================================="
print_status "Sync and merge complete!"
echo ""
